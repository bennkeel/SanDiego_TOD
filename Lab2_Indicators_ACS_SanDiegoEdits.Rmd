...
#plot the max buffer to check
ggplot() +
  geom_sf(data=trolleyMaxBuffer) +
  geom_sf(data=SD_trolley, show.legend = "point") +
  facet_wrap(~Legend) + 
  labs(caption = "Figure 2.6") +
  mapTheme()

#Intersect the tracts with the max buffer
selectionMax <- tracts16[st_intersects(tracts16, trolleyMaxBuffer) %>% lengths > 0, ] %>%
...
intersections <- rbind(selectionMax, selectMaxCentroids)

#Plot the intersection to see the shape
ggplot() +
  geom_sf(data=selectMaxCentroids, aes(fill = q5(TotalPop))) +
  geom_sf(data=SD_trolley, show.legend = "point") +
  scale_fill_manual(values = palette5,
    labels = qBr(totalPop16, "estimate"),
    name = "Popluation\n(Quintile Breaks)") +
