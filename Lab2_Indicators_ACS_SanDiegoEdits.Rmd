...
#Writing GeoJson for visualization/diagram purposes
...
tracts16_sf = st_as_sf(tracts16_slim, coords = c("x", "y"), crs = "EPSG:4326", agr = "constant")

st_write(tracts16_sf, "SDtracts2.geojson")
...
#Intersect the tracts with the max buffer //Not Doing This One
# selectionMax <- allTracts[st_intersects(allTracts, trolleyMaxBuffer) %>% lengths > 0, ] %>%
...
intersections <- rbind(selectionMax, tracts16_SD)
...
allTracts_LimitSD.rings <-
  ...
  st_sf() %>%
  mutate(distance = distance / 805) #convert to miles
...
```{r}
#allTracts <- rbind(tracts20_LimitSD,tracts16_LimitSD)
trolleyBuffersInd <- filter(trolleyBuffers, Legend == "Buffer")
allTracts_LimitSD_C <- dplyr::select(allTracts_LimitSD, GEOID, TotalPop, MedRent, year, geometry)
  
  
stationData <- 
  st_join(trolleyBuffersInd, allTracts_LimitSD_C, join = st_intersects) %>% #get spacial join/filter of Trolley Buffers and all_tracts_limitSD
  
  aggregate(stationData$totalPop, list(stationData$geometry), mean) 
    [trolleyBuffersInd,]%>% #get buffers and filter out unifier
  mutate()#give each buffer its associated tracts
  
  dplyr::select(tracts)
  mutate#average the population data (mutate?) 
  #average the rent data across the included tracts (mutate?)
  #join allTracts_LimitSD to station data based on intersecting each buffer's spatial-selected tracts
  st_drop_geometry() %>% #drop buffer geometry
  left_join(dplyr::select(SD_trolley, -route_id, -stop_agency)) %>% #add point geometry from SD_trolley
